...
Надо правила написания бота сделать
...
client_cerver_sock.py - версия для игры с сервером через сокеты (клиетнтская часть):
	...
	- принимает координаты x и y (1 <= x,y <= 10)
	...
	- принимает координаты выстрела a,b (1 <= x,y <= 10)
	- возвращает:
		...
		9) 8, если посланные координаты неправильные
	...
	- возвращает:
		...
		9) Неверный формат запроса, если посланные координаты неправильные
	- не используется в этом файле необходимо вызвать для получения ответа сервера на запрос
...
send_pic_to_server.py - отправка картинки (серверная часть)
	
	def get_null_string(gg): 
		- дополняет строку gg 0 сначала до длины 10
		- принимает строку 
		- возвращает строку заполенную нулями
		- используется в get_pref(index, length)
	
	def get_pref(index, length): 
		- создает заголовок пакета <позиция><длина>
		- принимает index позицию и length длину
		- возвращает заголовок
		- используется в send_to_client1(msg, index)
	
	def send_to_client1(msg, index): 
		- отправляет длинное сообщение msg клиенту с номером index
		- принимает  сообщение и номер
		- возвращает "*", если не удалось отправить
		- используется в send_pic(file_name, index)
	
	def send_pic(file_name, index):
		- отправляет файл file_name клиенту с номером index выводит, что клиент не получил сообщение если не удалось отправить
		- принимает имя файлаи индекс клиента
		- возвращает "*", если не удалось отправить
		- испотльзуется в main_server_sock.py
	

файлы test_*.py используются для тестирования

test_client_server.py - пример написания бота (используется для тестирования) (клиентская часть)

остальные файлы не являются актуальными использовались ранее для тестирования
